...
import { AlertTriangle, ArrowRight, BarChart3, CalendarClock, LogOut, RefreshCw, TrendingDown, TrendingUp, UsersRound } from "lucide-react";
import type { AppScreen, Crew, LeaderUser, Worker } from "@/data/agronexData";
import { calculateCrewProductivity, normalizeWorkerAttendance } from "@/lib/agronex-productivity";
...
import { InfoStat, ProgressBar, ScreenHeading, SectionTitle, StatusBadge } from "./ui";
import { CrewSummary, RegisterProgressForm, WorkerQuickEditor, type ProgressSubmission } from "./register-form";
...
export function LeaderHome({ leader, crew, workers, onNavigate }: LeaderProps & { onNavigate: (screen: AppScreen) => void }) {
  ...
  const summary = calculateCrewProductivity(taskCrew, dayWorkers);

  if (scope === "Quincena") {
    return <div className="space-y-5"><ScreenHeading eyebrow="Mi jornada" title={`Buenos días, ${leader.name}`} description="Resumen de tu cuadrilla y rendimiento acumulado." /><ScopeChips value={scope} onChange={setScope} /><FortnightSummary leader={leader} /></div>;
  }

  if (scope === "Ayer") {
    const yesterday = addDays(operationalDate, -1);
    const record = getLatestProgressForLeaderDate(progressRecords, leader.id, yesterday);
    return <div className="space-y-5"><ScreenHeading eyebrow="Registro histórico" title={`Jornada ${formatDateLabel(yesterday)}`} description="Consulta de solo lectura para días anteriores." /><ScopeChips value={scope} onChange={setScope} />{record ? <HistoricProgressCard record={record} /> : <EmptyState title="Sin registros para esta fecha." description="Cuando guardes avances, el historial aparecerá aquí." />}</div>;
  }

  return <div className="space-y-5"><ScreenHeading eyebrow="Mi jornada" title={`Buenos días, ${leader.name}`} description={todayTask ? "Tarea de hoy lista para registrar avance." : "Tu perfil y cuadrilla siguen disponibles."} /><ScopeChips value={scope} onChange={setScope} />{!todayTask ? <><EmptyState title="Sin tarea asignada para hoy" description="Cuando el supervisor planifique, aparecerá aquí." /><section className="ag-card p-5"><SectionTitle title="Mi cuadrilla" /><div className="grid grid-cols-2 gap-3"><InfoStat label="Cuadrilla" value={crew.name} /><InfoStat label="Trabajadores" value={String(workers.length)} /><InfoStat label="Labor base" value={leader.labor} /><InfoStat label="Sector" value={leader.sector} /></div></section><FortnightSummary leader={leader} compact /></> : <><section className="rounded-3xl bg-[linear-gradient(135deg,#174d38,#267852)] p-5 text-white shadow-lg shadow-[#164a35]/15"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-widest text-white/55">Tarea de hoy</p><h2 className="mt-2 text-lg font-extrabold leading-snug">{taskCrew.labor} · {taskCrew.sector}</h2><p className="mt-1 text-xs text-white/65">{taskCrew.crop} · {taskCrew.name}</p></div><StatusBadge status={taskCrew.status} /></div><div className="mt-5"><div className="flex items-center justify-between text-xs font-bold text-white/75"><span>Avance</span><span className="tabular-nums">{summary.percentage}%</span></div><div className="mt-2"><ProgressBar value={summary.percentage} color="#bce8cb" /></div></div><div className="mt-5 grid grid-cols-3 gap-2 rounded-2xl bg-white/10 p-3"><HeroStat label="Meta" value={`${taskCrew.goal} ${taskCrew.unit}`} /><HeroStat label="Avance" value={`${summary.progress} ${taskCrew.unit}`} /><HeroStat label="Falta" value={`${summary.remaining} ${taskCrew.unit}`} /></div></section><CrewSummary summary={summary} crew={taskCrew} /><section className="ag-card p-5"><SectionTitle title="Avance del día" /><div className="mt-3 grid grid-cols-3 gap-3"><InfoStat label="Meta" value={`${taskCrew.goal} ${taskCrew.unit}`} /><InfoStat label="Realizado" value={`${summary.progress} ${taskCrew.unit}`} /><InfoStat label="Pendiente" value={`${summary.remaining} ${taskCrew.unit}`} /></div><button onClick={() => onNavigate("registrar")} className="ag-primary mt-6 w-full">Registrar avance <ArrowRight size={17} /></button></section></>}</div>;
}
...
export function LeaderRegister({ leader, crew, workers, onSave }: LeaderProps & { onSave: (submission: ProgressSubmission) => void }) {
  ...
  if (!task) return <div><ScreenHeading eyebrow="Registro de campo" title="Guardar avance" description="El registro se habilita cuando exista una tarea activa para hoy." /><EmptyState title="No tienes tarea asignada para hoy." description="Cuando el supervisor planifique, aparecerá aquí." /></div>;

  return <div><ScreenHeading eyebrow="Registro de campo" title="Guardar avance" description="Marca asistencia y registra avance por trabajador." /><RegisterProgressForm leader={leader} crew={mergeCrewWithDay(crew, task, progress)} initialWorkers={progress?.workers ?? workers} onSave={onSave} /></div>;
}

export function MyCrewScreen({ leader, crew, workers }: LeaderProps) {
  const summary = calculateCrewProductivity(crew, workers);
  return <div><ScreenHeading eyebrow="Equipo asignado" title="Mi cuadrilla" description="Asistencia y productividad de tu equipo durante la jornada." action={<AddWorkerButton leader={leader} crew={crew} />} /><CrewSummary summary={summary} crew={crew} /><section className="mt-5"><SectionTitle title="Trabajadores de la cuadrilla" /><WorkerList workers={workers} crew={crew} leader={leader} /></section></div>;
}

export function MyWorkersScreen({ leader, crew, workers }: LeaderProps) {
  const [filter, setFilter] = useState("Todos");
  const summary = calculateCrewProductivity(crew, workers);
  const visible = workers.filter((worker) => filter === "Todos" || (filter === "Presentes" && worker.attendance === "Presente") || (filter === "Ausentes" && worker.attendance === "Ausente") || (filter === "Alto rendimiento" && worker.attendance === "Presente" && worker.dailyOutput >= summary.averagePerWorker) || (filter === "Bajo rendimiento" && worker.attendance === "Presente" && worker.dailyOutput < summary.averagePerWorker));
  return <div><ScreenHeading eyebrow="Mi equipo" title="Mis trabajadores" description={`Solo trabajadores de ${crew.name}.`} action={<AddWorkerButton leader={leader} crew={crew} />} /><CrewSummary summary={summary} crew={crew} /><div className="mb-5 mt-5 flex gap-2 overflow-x-auto pb-1">{["Todos", "Presentes", "Ausentes", "Alto rendimiento", "Bajo rendimiento"].map((item) => <button key={item} onClick={() => setFilter(item)} className={`min-h-10 whitespace-nowrap rounded-full px-4 text-xs font-bold ${filter === item ? "bg-[#1a5b40] text-white" : "border border-[#dfe7e1] bg-white text-[#587066]"}`}>{item}</button>)}</div><WorkerList workers={visible} crew={crew} leader={leader} /></div>;
}

export function LeaderPerformance({ leader, crew, workers }: LeaderProps) {
  const { operationalDate } = useAgroSession();
  const summary = calculateCrewProductivity(crew, workers);
  const ranked = [...workers].filter((worker) => worker.attendance === "Presente").sort((a, b) => b.dailyOutput - a.dailyOutput);
  return <div><ScreenHeading eyebrow="Análisis personal" title="Rendimiento" description="Cumplimiento y productividad de tu cuadrilla." /><div className="grid gap-5 lg:grid-cols-2"><CrewSummary summary={summary} crew={crew} /><FortnightSummary leader={leader} date={operationalDate} /><RankingCard title="Mejores trabajadores" icon={TrendingUp} workers={ranked.slice(0, 5)} crew={crew} /><RankingCard title="Trabajadores con bajo avance" icon={TrendingDown} workers={ranked.slice(-5).reverse()} crew={crew} /></div><section className="mt-5 rounded-2xl bg-[#173c2d] p-5 text-white"><div className="flex gap-3"><AlertTriangle className="shrink-0 text-[#f2c76e]" size={20} /><p className="text-sm font-semibold leading-6">Revisar distribución de personal. La cuadrilla tiene {summary.totalWorkers} trabajadores y {summary.presentWorkers} presentes. El rendimiento por hora hombre es {summary.outputPerManHour} {crew.unit}/hh.</p></div></section></div>;
}
...
function WorkerList({ workers, crew, leader }: { workers: Worker[]; crew: Crew; leader: LeaderUser }) {
  const { updateWorker } = useAgroSession();
  const [dirtyWorkers, setDirtyWorkers] = useState<Record<string, Worker>>({});
  const visibleWorkers = workers.map((worker) => dirtyWorkers[worker.id] ?? worker);

  const changeWorker = (worker: Worker) => {
    const normalized = normalizeWorkerAttendance(worker, worker.attendance);
    setDirtyWorkers((current) => ({ ...current, [normalized.id]: normalized }));
    updateWorker(normalized);
  };

  if (visibleWorkers.length === 0) return <EmptyState title="No hay trabajadores asignados a esta cuadrilla." description="Agrega trabajadores para comenzar el registro del día." />;
  return <div className="grid gap-3 md:grid-cols-2">{visibleWorkers.map((worker) => <WorkerQuickEditor key={worker.id} worker={worker} onChange={changeWorker} action={<WorkerActionMenu worker={worker} leaderScope={{ leader, crew }} />} />)}</div>;
}
...
function FortnightSummary({ leader, date, compact = false }: { leader: LeaderUser; date?: string; compact?: boolean }) {
  ...
  return <section className="ag-card p-5"><SectionTitle title="Mi rendimiento quincenal" /><p className="mt-1 text-xs font-semibold text-[#718078]">{range.start} al {range.end}</p>{records.length === 0 ? <EmptyState title="Sin registros en esta quincena." description="Registra avances diarios para construir tu rendimiento acumulado." dense /> : <><div className={`mt-4 grid gap-3 ${compact ? "grid-cols-2" : "grid-cols-2 sm:grid-cols-3"}`}><InfoStat label="Días registrados" value={String(new Set(records.map((record) => record.date)).size)} /><InfoStat label="Meta acumulada" value={`${formatNumber(goal)} ${records[0]?.unit ?? ""}`} /><InfoStat label="Avance acumulado" value={`${formatNumber(progress)} ${records[0]?.unit ?? ""}`} /><InfoStat label="Faltante" value={`${formatNumber(pending)} ${records[0]?.unit ?? ""}`} /><InfoStat label="Horas" value={`${formatNumber(hours)} h`} /><InfoStat label="Cumplimiento" value={`${percentage}%`} /></div><div className="mt-4"><ProgressBar value={percentage} /></div>{!compact && <div className="mt-4 grid gap-3 sm:grid-cols-2"><InfoStat label="Mejor día" value={best ? `${best.date} · ${best.progress} ${best.unit}` : "Sin registro"} /><InfoStat label="Menor avance" value={lowest ? `${lowest.date} · ${lowest.progress} ${lowest.unit}` : "Sin registro"} /></div>}</>}</section>;
}
...
function mergeCrewWithDay(crew: Crew, task: LocalPlanningRecord, progress: LocalProgressRecord | null): Crew {
  ...
  const percentage = task.goal > 0 ? Math.round((done / task.goal) * 100) : 0;
  const workers = progress?.workers ?? [];
  const present = workers.filter((worker) => worker.attendance === "Presente");

  return { ...crew, labor: task.labor, crop: task.crop as Crew["crop"], sector: task.sector, goal: task.goal, progress: done, remaining, unit: task.unit, percentage, presentWorkers: progress ? present.length : crew.presentWorkers, absentWorkers: progress ? workers.length - present.length : crew.absentWorkers, manHours: progress ? workers.reduce((sum, worker) => sum + worker.hoursWorked, 0) : crew.manHours, status: percentage >= 100 ? "Terminado" : done > 0 ? "En proceso" : "Pendiente" };
}
